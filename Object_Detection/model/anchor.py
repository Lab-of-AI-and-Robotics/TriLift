...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, conv_depth=1, rotate=False):
        super().__init__()
        # in_channels *= 3 ## 따로 해주기 위해
        conv_depth = 1 ##conv3d operation n time 
        convs = []
    ...
    def forward(self, x: List[Tensor]) -> Tuple[List[Tensor], List[Tensor]]:
        ...
        for i, feature in enumerate(x):
            t = self.conv(feature)
            cls_logit = self.cls_logits(t)
            bbox_pred = self.bbox_pred(t)
            logits.append(cls_logit)
            bbox_reg.append(bbox_pred)

        # 최종적으로 각 레벨별 출력 텐서들을 리스트로 반환합니다.
        return logits, bbox_reg


    # def forward(self, x: List[Tensor]) -> Tuple[List[Tensor], List[Tensor]]:
    #     # 재귀 프로파일링 방지를 위한 플래그 체크
    #     if not (hasattr(self, "_in_profile") and self._in_profile) and not hasattr(self, "_flops_printed"):
    #         # 각 입력 텐서를 clone하여 원본 입력 저장
    #         orig_input = [item.clone() for item in x]
    #         self._in_profile = True  # 재귀 호출 시 플래그 활성화
    #         from thop import profile
    #         flops, params = profile(self, inputs=(orig_input,), verbose=False)
    #         print(f"RPNHead FLOPs: {flops}")
