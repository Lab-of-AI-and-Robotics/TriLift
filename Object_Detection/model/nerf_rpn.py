...
class NeRFRegionProposalNetwork(nn.Module):
    ...
    def _measure_flops_once(self, mesh_tensors, features, original_mesh_sizes, targets=None):
        """한 번만 실행하여 RPN head의 FLOPs를 측정하고 결과를 출력하는 메서드"""
        if not THOP_AVAILABLE:
            print("Warning: thop not available. RPN head FLOPs measurement skipped.")
            return
        ...
        try:
            # RPN head만 따로 측정
            rpn_head = self.rpn.head
            
            # 각 레벨별 feature에 대해 FLOPs 측정
            for i, feature in enumerate(features):
                # Conv layer 측정
                conv_flops, _ = profile(rpn_head.conv, inputs=(feature,), verbose=False)
                conv_output = rpn_head.conv(feature)
                
                # Classification head 측정
                cls_flops, _ = profile(rpn_head.cls_logits, inputs=(conv_output,), verbose=False)
                
                # Bbox regression head 측정
                bbox_flops, _ = profile(rpn_head.bbox_pred, inputs=(conv_output,), verbose=False)
                
                # 레벨별 FLOPs 합산
                level_flops = conv_flops + cls_flops + bbox_flops
                step_flops[f'rpn_head_level_{i}'] = level_flops
                total_flops += level_flops
                
                # Level {i}: feature shape {feature.shape}
            
            step_flops['rpn_head_total'] = total_flops
            
        except Exception as e:
            print(f"RPN head FLOPs 측정 중 오류 발생: {e}")
            return
        
        # 결과 출력
        print(f"\n=== RPN Head FLOPs ===")
        print(f"Total RPN Head FLOPs: {total_flops/1e9:.2f} G")
        print(f"======================\n")
    ...
    def forward(self, meshes, targets=None, objectness_output_paths=None):
        ...
        # RPN head FLOPs 측정 (한 번만 실행, training 모드에서만)
        if THOP_AVAILABLE and not self._flops_measured and self.training:
            self._measure_flops_once(mesh_tensors, features, original_mesh_sizes, targets)
