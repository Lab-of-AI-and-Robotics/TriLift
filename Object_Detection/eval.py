...
def evaluate_box_proposals_f1_score(proposals_list, proposal_scores_list, gt_boxes_list, 
                                   iou_thresh=0.25, score_thresh=0.5, top_k=None):
    """
    Evaluate F1-score for object detection proposals
    """
...
def evaluate_box_proposals_f1_multiple_thresholds(proposals_list, proposal_scores_list, gt_boxes_list, 
                                                 iou_thresholds=[0.25, 0.35, 0.5], 
                                                 score_thresholds=[0.1, 0.3, 0.5], 
                                                 top_k=None):
    """
    Evaluate F1-score for multiple IoU and confidence thresholds
    """
    results = {}
    
    for iou_thresh in iou_thresholds:
        for score_thresh in score_thresholds:
            f1_result = evaluate_box_proposals_f1_score(
                proposals_list, proposal_scores_list, gt_boxes_list,
                iou_thresh=iou_thresh, score_thresh=score_thresh, top_k=top_k
            )
            
            key = f"IoU{int(iou_thresh*100)}_Score{int(score_thresh*100)}"
            results[key] = f1_result
