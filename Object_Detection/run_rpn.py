...
try:
    ...
    print("Warning: thop not available. FLOPs measurement will be skipped.")
import numpy as np
import torch
...
from model.feature_extractor import (Total_model, VGG_FPN, Bottleneck, ResNet_FPN_64,
...
def parse_args():
    ...
    parser.add_argument('--pe_type', type=str, default='transformer',
                        choices=['transformer'],
                        help='Positional encoding type (transformer only)')
    parser.add_argument('--3dratio', type=float, default=0.5,
                        help='Low-resolution volumetric branch ratio (default: 0.5; use 0.25 for quarter resolution)')
...
class Trainer:
    def __init__(self, args, rank=0, world_size=1, device_id=None, logger=None):
        ...
        self.logger.info(f"Constructing TriLift model with 3D ratio {ratio_3d} ({int(args.resolution * ratio_3d)}×{int(args.resolution * ratio_3d)}×{int(args.resolution * ratio_3d)}).")
        ...
        self.model = self.NeRFRegionProposalNetwork(
            self.backbone, 
    ...
    def eval_checkpoints(self):
        # Get all checkpoint files in the save_path directory
        checkpoint_files = sorted(glob.glob(os.path.join(self.args.save_path, '*.pt')), 
                                 key=lambda x: int(re.search(r'epoch_(\d+)\.pt', os.path.basename(x)).group(1)))
        ...
        for checkpoint_file in checkpoint_files:
            self.logger.info(f'Evaluating checkpoint: {checkpoint_file}')
            checkpoint = torch.load(checkpoint_file)
            
            # Load state dicts with strict=False to handle FLOPs measurement keys (total_ops, total_params)
            self.backbone.load_state_dict(checkpoint['backbone_state_dict'], strict=False)
            self.rpn_head.load_state_dict(checkpoint['rpn_head_state_dict'], strict=False)

            eval_results = self.eval(self.test_set)

            # Add results to the consolidated dictionary
            checkpoint_name = os.path.basename(checkpoint_file)
            all_results[checkpoint_name] = eval_results

            # Extract recall50 from Top 2500 proposals
            if eval_results and 'Top 2500 proposals' in eval_results:
                recall50_str = eval_results['Top 2500 proposals']
                # Extract recall50 value using regex
                match = re.search(r'Recall@50: Recall: ([\d.]+)', recall50_str)
                if match:
                    recall50_value = float(match.group(1))
                    if recall50_value > best_recall50:
                        best_recall50 = recall50_value
                        best_result = eval_results
                        best_checkpoint = checkpoint_name

        # Add best result based on recall50
        if best_result is not None:
            all_results['best_recall50'] = {
                'checkpoint': best_checkpoint,
                'recall50_value': best_recall50,
                'results': best_result
            }

        # Save all results to a single json file
        result_file = os.path.join(self.args.save_path, 'result.json')
    ...
    def build_backbone(self):
        # Import the TriLift (ours) modules
        from model.feature_extractor import (Total_model, VGG_FPN, Bottleneck,
                                             ResNet_FPN_64, ResNet_FPN_256,
                                             ResNetSimplified_64, ResNetSimplified_256,
                                             SwinTransformer_FPN)
        from model.nerf_rpn import NeRFRegionProposalNetwork

        # Store the NeRFRegionProposalNetwork for later use
        self.NeRFRegionProposalNetwork = NeRFRegionProposalNetwork
        ...
        if self.args.backbone_type == 'resnet':
            ...
            self.backbone = Total_model("AF", 4, True, self.args.resolution, self.args.pe_type, ratio_3d=args.__dict__.get('3dratio', 0.5))
        elif self.args.backbone_type == 'vgg_EF':
            # Use Total_model (TriLift backbone) with PE support
            self.backbone = Total_model("EF", 4, True, self.args.resolution, self.args.pe_type, ratio_3d=getattr(self.args, '3dratio', 0.5))
    ...
    def train_loop(self):
        ...
        for epoch in range(1, self.args.num_epochs + 1):
            ...
            #     eval_results = self.eval(self.val_set)  # 수정된 부분
            #     recalls = eval_results['Top 2500 proposals'].split('\n')[-1].split(': ')[-1]  # 수정된 부분
            #     metric = float(recalls)  # 수정된 부분
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # print("Input RGBSigma shapes:", [item.shape for item in rgbsigma])
            # print("GT Boxes shapes:", [item.shape for item in boxes])
            if torch.cuda.is_available():
    ...
    def eval(self, dataset):
        ...
        for limit in [300, 1000, self.args.rpn_post_nms_top_n_test]:
            ...
            json_dict[f'Top {limit} proposals'] = (
        ...
        json_dict['AP@50'] = f'AP: {ap50["ap"].item():.4f}'
        json_dict['AP@25'] = f'AP: {ap25["ap"].item():.4f}'
    ...
    if args.gpus:
        ...
        if len(gpu_ids) <= 1:
            ...
            if args.log_to_file:
                ...
                file_handler.setFormatter(logging.Formatter('[%(asctime)s %(levelname)s] %(message)s'))  # 수정된 부분
                file_handler.setLevel(logging.DEBUG)
